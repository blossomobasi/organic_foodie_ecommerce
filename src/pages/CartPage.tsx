...
const CartPage = () => {
    ...
    // const products = localStorage.getItem("products") as ProductResponse | null;
    const { products } = useProducts();
    const { cart } = useCart();

    const productInCart = cart?.userOrdersCart.map((cart) => {
        const cartProductId = cart.products.map((product) => product.productId);

        for (let i = 0; i < cartProductId.length; i++) {
            return cartProductId.map((el) => {
                products?.allProduct.map((product) => product._id === el);
            });
        }
    });
